...
cv::Mat matFromImage(const shm_msgs::msg::Image6m & source)
{
  int source_type = getCvType(shm_msgs::get_str(source.encoding));
  int byte_depth = enc::bitDepth(shm_msgs::get_str(source.encoding)) / 8;
  int num_channels = enc::numChannels(shm_msgs::get_str(source.encoding));
  ...
  if (source.height * source.step > shm_msgs::msg::Image6m::DATA_MAX_SIZE) {
    std::stringstream ss;
    ss << "Image is wrongly formed: height * step > size  or  " << source.height << " * " <<
      source.step << " > " << shm_msgs::msg::Image6m::DATA_MAX_SIZE;
    throw Exception(ss.str());
  }
...
shm_msgs::msg::Image6m::SharedPtr CvImage::toImageMsg6m() const
{
  shm_msgs::msg::Image6m::SharedPtr ptr = std::make_shared<shm_msgs::msg::Image6m>();
  toImageMsg(*ptr);
  return ptr;
}
...
void CvImage::toImageMsg(shm_msgs::msg::Image6m & ros_image) const
{
  shm_msgs::set_header(ros_image.header, header);
  ros_image.height = image.rows;
  ros_image.width = image.cols;
  ...
  size_t size = ros_image.step * image.rows;
  // ros_image.data.resize(size);
  if (size > shm_msgs::msg::Image6m::DATA_MAX_SIZE) {
    std::stringstream ss;
    ss << "Image is wrongly formed: height * step > size  or  " << ros_image.height << " * " <<
      ros_image.step << " > " << shm_msgs::msg::Image6m::DATA_MAX_SIZE;
    throw Exception(ss.str());
  }
...
CvImagePtr toCvCopy(
  const shm_msgs::msg::Image6m::ConstSharedPtr & source,
  const std::string & encoding)
{
  return toCvCopy(*source, encoding);
}
...
CvImagePtr toCvCopy(
  const shm_msgs::msg::Image6m & source,
  const std::string & encoding)
{
  // Construct matrix pointing to source data
  return toCvCopyImpl(matFromImage(source), shm_msgs::get_header(source.header), shm_msgs::get_str(source.encoding), encoding);
